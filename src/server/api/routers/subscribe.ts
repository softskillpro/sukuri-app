...
export const subscriptionRouter = createTRPCRouter({
    ...
    subscribe: protectedProcedure.input(SubscriptionInput).mutation(async ({ input, ctx }) => {        
        ...
        const tier = await ctx.prisma.projectTier.findUnique({ where: { tier_id: tierId } });
        if (!tier || tier.projectId !== projectId) {
            throw new Error('Mismatched IDs: The tier does not belong to the specified project');
        }
        ...
        if (existingSubscription) {
            throw new Error('Already Subscribed: You are already subscribed to this tier');
        }
