...
type SubscribedData = z.infer<typeof subscribedSchema>;
...
export default async function handle(
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (userId !== userId) {
        // stand in for if we add userId in the subscribe api call
      ...
      const tier = await findOrThrow(
        prisma.projectTier.findUnique({ where: { tier_id: tierId } }),
        'Tier not found',
      );
    ...
    const parsedData = subscribedSchema.safeParse(req.query);
    if (!parsedData.success) {
      return res.status(400).json({
        error: 'Invalid request data',
        details: (parsedData.error as ZodError).errors,
    ...
    const { projectId } = parsedData.data as SubscribedData;
    ...
    const isSubscribed = !!userProject;

    res.status(200).json({
      success: true,
      message: 'Checked subscription successfully',
      isSubscribed,
    });
  } else {
    res.status(405).json({
